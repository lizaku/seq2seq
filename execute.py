...
try:
    ...
    from configparser import \
        SafeConfigParser  # In Python 3, ConfigParser has been renamed to configparser for PEP 8 compliance.
...
def create_model(session, forward_only):
    ...
    model = seq2seq_model.Seq2SeqModel(gConfig['enc_vocab_size'], gConfig['dec_vocab_size'], _buckets,
                                       ...
                                       forward_only=forward_only, use_lstm=bool(gConfig['use_lstm']))
...
def train():
    ...
    config = tf.ConfigProto(gpu_options=gpu_options, inter_op_parallelism_threads=NUM_CORES,
...
def decode():
    ...
    with tf.Session(config=config) as sess:
        ...
        while sentence:
            ...
            token_ids = data_utils.sentence_to_token_ids(sentence.lower(), enc_vocab)
